---@diagnostic disable: undefined-global, need-check-nil, undefined-field, assign-type-mismatch
...
---@alias LocalizedOptionValue { text: string, data: string }
--- text: localized string
--- data: value

---@alias WorldOption string[]
---@alias OptionRemap { img: string, atlas: string }
---@alias Option { value: string, desc: LocalizedOptionValue[]?, image: string, world: WorldOption, options_remap: OptionRemap?, name: string?, master_group: MasterGroup?, group: Group?, category: LevelCategory? }
---@alias Group { order: integer, text: string, desc: LocalizedOptionValue[]?, atlas: string, items: table<string, Option>, master_group: MasterGroup?, group_name: string?, category: LevelCategory? }
---@alias MasterGroup table<string, Group>

---@alias OptionElement { name: string, master_group: MasterGroup, group: Group, category: LevelCategory, value: string, image: string, options_remap: OptionRemap?, desc: LocalizedOptionValue[]?, world: WorldOption?, masteroption: boolean?, master_controlled: boolean?, master_sync: boolean? }
---@alias OptionList table<string, OptionElement>

-- ---------- ---------- ---------- ---------- ---------- ---------- ---------- ---------- ---------- ---------- --

---@type LocalizedOptionValue[]
...
for _, group_data in pairs(WORLDSETTINGS_GROUP) do
	for name in pairs(group_data.items) do
		local flag1 = WorldSettings_Overrides.Pre[name]
		local flag2 = WorldSettings_Overrides.Post[name]
		local flag3 = EXEMPT_OPTIONS[name]
		assert(flag1 or flag2 or flag3, "Missing WorldSettings_Override for customization option "..name)
	end
end
...
local function IsGroupDisabled(category, groupname)
	return MOD_DISABLE_GROUP[category] and
			MOD_DISABLE_GROUP[category][groupname] and
			not IsTableEmpty(MOD_DISABLE_GROUP[category][groupname])
end
